...
public class AlertasActivity extends AppCompatActivity {
    ...
    private void getAlertas() {
        ...
        try{
            System.out.println(alertas!=null);
            if(alertas!=null){
                for (int i=0;i< alertas.length();i++){

                    JSONObject c = alertas.getJSONObject(i);
                    // String zona = c.getString("Zona");
                    String sensor = c.getString("IDSensor");

                    String hora = c.getString("Data"); //NÃO PODE ESTAR HORAESCRITA
                    double leitura;
                    try {
                        leitura = c.getDouble("Medicao");
                    } catch (Exception e) {
                        leitura = -1000.0;
                    }
                    String tipoAlerta = c.getString("NivelAlerta");
                    //String cultura = c.getString("IDCultura");
                   ...
                   /* int idUtilizador;
                    ...
                    db.insertAlerta( sensor, hora, leitura, tipoAlerta, mensagem, idCultura, horaEscrita);
                }
            }
    ...
    private void listAlertas() {
        ...
        List<String> alertaFields = Arrays.asList(new String[]{ "Sensor", "Hora", "Leitura", "TipoAlerta", "Mensagem" , "IDCultura", "HoraEscrita"});
